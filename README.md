...
| `Channel::WhatsAppInbound` | `whatsapp_inbound` | `requestOtp()` only; user messages OTP.ID |

### Bring your own code

```php
$res = $client->sendOtp('482913', new OrderParams(
    channel: Channel::Sms,
    destination: '6281234567890',
));
```
...
```php
$res = $client->requestOtp(new OrderParams(channel: Channel::WhatsAppInbound));
